...
def cceval_generate(
    ...
    for d in data:
        ...
        contexts = [d['crossfile_context']['text']] +[chunk['retrieved_chunk'] for chunk in d['crossfile_context']['list']]
    ...
    with open(out_path, 'w') as f:
        ...
        for d in data:
            ...
            d_base['context_type'] = 'base_prompt'
            d_base['task_id'] = d['metadata']['task_id']
            print(json.dumps(d_base), file=f, flush=True)
            idx += 1
            contexts = ['full_context'] + [f"chunk_{i}" for i in range(len(d['crossfile_context']['list']))]
            for context_type in contexts:
                d_modified = d.copy()
                d_modified['pred'] = outputs[idx].outputs[0].text
                d_modified['context_type'] = context_type
                d_modified['task_id'] = d['metadata']['task_id']
                print(json.dumps(d_modified), file=f, flush=True)
                idx += 1
